...
-- Fix user_roles: make policies PERMISSIVE so admin check works
...
CREATE POLICY "Anyone can read analysis cache"
  ON public.daily_analysis_cache FOR SELECT TO public
  USING (true);
...
CREATE POLICY "Service role can manage analysis cache"
  ON public.daily_analysis_cache FOR ALL TO service_role
...
CREATE POLICY "Anyone can view tracked picks"
  ON public.tracked_picks FOR SELECT TO public
  USING (true);
...
CREATE POLICY "Service role can manage tracked picks"
  ON public.tracked_picks FOR ALL TO service_role
  USING (true) WITH CHECK (true);
...
CREATE POLICY "Anyone can view pick snapshots"
  ON public.pick_snapshots FOR SELECT TO public
  USING (true);
...
CREATE POLICY "Service role can manage pick snapshots"
  ON public.pick_snapshots FOR ALL TO service_role
  USING (true) WITH CHECK (true);
...
CREATE POLICY "Users can manage own alerts"
  ON public.price_alerts FOR ALL TO public
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Service role manages watchlist alert state"
  ON public.watchlist_alert_state FOR ALL TO service_role
  USING (true) WITH CHECK (true);
...
CREATE POLICY "Users can view own alert state"
  ON public.watchlist_alert_state FOR SELECT TO authenticated
  USING (auth.uid() = user_id);
...
CREATE POLICY "Authenticated users can view approved digests"
  ON public.market_digests FOR SELECT TO public
  USING ((status = 'approved'::text) OR has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Admins can manage digests" ON public.market_digests;
CREATE POLICY "Admins can manage digests"
  ON public.market_digests FOR ALL TO public
  USING (has_role(auth.uid(), 'admin'::app_role))
  WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
