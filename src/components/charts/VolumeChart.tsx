...
export default function VolumeChart({ data }: Props) {
  ...
  return (
    <div className="bg-sf-card border border-border rounded-xl p-4">
      <h3 className="text-foreground font-semibold text-sm mb-3">Volume</h3>
      <ResponsiveContainer width="100%" height={120}>
        <BarChart data={chartData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
          ...
          <YAxis tick={{ fill: 'hsl(213 20% 55%)', fontSize: 9, fontFamily: 'JetBrains Mono' }} axisLine={false} tickFormatter={(v: number) => fmtCompact(v)} />
