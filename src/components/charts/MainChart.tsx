...
export default function MainChart({ data }: Props) {
  const { prices, indicators, forecast, forecastTarget, marketPhase } = data;
  ...
  return (
    <div className="bg-sf-card border border-border rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-foreground font-semibold text-sm">Price Chart</h3>
      ...
      <ResponsiveContainer width="100%" height={350}>
        <ComposedChart data={combined} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
          ...
          <XAxis dataKey="time" tickFormatter={formatTime} tick={{ fill: 'hsl(213 20% 55%)', fontSize: 10, fontFamily: 'JetBrains Mono' }} axisLine={{ stroke: 'hsl(216 30% 18%)' }} />
          <YAxis domain={['auto', 'auto']} tick={{ fill: 'hsl(213 20% 55%)', fontSize: 10, fontFamily: 'JetBrains Mono' }} axisLine={{ stroke: 'hsl(216 30% 18%)' }} tickFormatter={(v: number) => fmtPrice(v).replace('$', '')} />
          <Tooltip
